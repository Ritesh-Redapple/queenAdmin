...
import { NlhCreateUpdateComponent } from './nlh-create-update/nlh-create-update.component';

import { first , tap, delay,map } from 'rxjs/operators';
import { NLHService } from '../services/nlh.service';
import { AuthService } from '../auth/auth.service';
import { MatSnackBar } from '@angular/material/snack-bar';
@Component({
  selector: 'vex-nlh-tables',
  templateUrl: './nlh-tables.component.html',
  styleUrls: ['./nlh-tables.component.scss'],
  animations: [
    fadeInUp400ms,
    stagger40ms
  ],
...
export class NlhTableComponent implements OnInit, AfterViewInit, OnDestroy {
...
{ label: 'Image', property: 'table_id', type: 'text', visible: false },*/


{ label: 'Room Name', property: 'room_name', type: 'text', visible: true },
{ label: 'Hand Count', property: 'hand_count', type: 'text', visible: true },
{ label: 'Minimum Level', property: 'minimum_level', type: 'text', visible: true },
{ label: 'Hand Time (Sec)', property: 'hand_time', type: 'text', visible: true },
{ label: 'Player Count', property: 'player_count', type: 'text', visible: true },
{ label: 'Game Running Status', property: 'is_gameRunning', type: 'text', visible: true },
{ label: 'Card Passing', property: 'card_passing', type: 'text', visible: true },
...
constructor(private dialog: MatDialog, private nlhService:NLHService,private location: Location,private snackbar: MatSnackBar) {
}
...
this.nlhService.getAllTables().subscribe(users => {
const room = users["data"];
this.players = room.map((val)=>{
  val.card_passing = val.card_passing.toUpperCase();
  val.is_gameRunning = (val.is_gameRunning == true) ? 'Yes' : 'No' ;
  return val;
})
...
createPlayer() {
console.log("pll")
this.dialog.open(NlhCreateUpdateComponent).afterClosed().subscribe((player: Player) => {
/**
...
this.dialog.open(NlhCreateUpdateComponent, {
data: player
...
deletePlayer(player: Player) {
   ...
   this.nlhService.deleteNlh(player).subscribe(user => {
     //console.log("kkkk"+JSON.stringify(user.error))
      if(!user["error"]){
